...
构建流程会在实例上检测并通过 apt-get 安装 Docker 与 Docker Compose 插件（需使用 Debian/Ubuntu 镜像并具备 root 权限），以确保 docker compose 可用。安装依赖系统默认的 apt 软件源与签名校验；如需自定义镜像或源，请确保 Docker 与 Compose 插件版本兼容且仓库可信。
